...
class Users extends Model
{
    ...
    public function validation()
    {
        ...
        if($this->IsAct && ($this->TestEnglish < 0 || $this->TestEnglish > 36)){
        ...
        if($this->IsAct && ($this->TestMath < 0 || $this->TestMath > 36)){
        ...
        if($this->IsAct && ($this->TestReading < 0 || $this->TestReading > 36)){
            $this->appendMessage(new Message("ACT Reading score must be between 0 and 36"));
        }

        if($this->IsAct && ($this->TestScience < 0 || $this->TestScience > 36)){
